...
class IntentHandler: INExtension, INSendMessageIntentHandling, INSearchForMessagesIntentHandling, INSetMessageAttributeIntentHandling, INSearchForPhotosIntentHandling {
	
	func handle(intent: INSearchForPhotosIntent, completion: @escaping (INSearchForPhotosIntentResponse) -> Void) {
		print("handle SearchForPhotosIntent")
		let userActivity = NSUserActivity(activityType: NSStringFromClass(INSearchForPhotosIntent.self))
//		userActivity.title = "OK"
		userActivity.userInfo = ["ids": [0, 1, 2]]
		
		let response = INSearchForPhotosIntentResponse(code: .continueInApp, userActivity: userActivity)
		response.searchResultsCount = 5
		
		completion(response)
	}
	
	func resolveSearchTerms(for intent: INSearchForPhotosIntent, with completion: @escaping ([INStringResolutionResult]) -> Void) {
		print("ResolveSearchTerms")
		let activity = NSUserActivity(activityType: "se.glennolsson.SwiftUI-testing.siri.searchPhoto")
		activity.title = "Hej hopp: "
...
//		activity.becomeCurrent()
		...
		let response = INStringResolutionResult.success(with: "hello!")
		completion([response])
	}
	...
	func confirm(intent: INSearchForPhotosIntent, completion: @escaping (INSearchForPhotosIntentResponse) -> Void) {
		...
		let userActivity = NSUserActivity(activityType: NSStringFromClass(INSearchForPhotosIntent.self))
		let response = INSearchForPhotosIntentResponse(code: .ready, userActivity: userActivity)
		completion(response)
	}
